...
void __fastcall TForm7::BitBtn1Click(TObject *Sender)
{
	...
	if (MaskEdit1->Text.Length() != 0 && Edit2->Text.Length() != 0 &&  DBLookupComboBox1->KeyValue != -1 && ComboBox1->ItemIndex != -1)
	{
			...
			for(Form1->DBGrid1->DataSource->DataSet->First(); !Form1->DBGrid1->DataSource->DataSet->Eof; Form1->DBGrid1->DataSource->DataSet->Next())
			{
				if(pib == Form1->DBGrid1->DataSource->DataSet->FieldByName("pib")->AsAnsiString)
				{
...
//---------------------------------------------------------------------------
void __fastcall TForm7::CheckBox1Click(TObject *Sender) //true/false image
{
...
//---------------------------------------------------------------------------
void __fastcall TForm7::BitBtn3Click(TObject *Sender)//open image player
{
...
void __fastcall TForm7::BitBtn2Click(TObject *Sender)
{
	...
	if (MaskEdit1->Text.Length() != 0  && Edit2->Text.Length() != 0 &&  DBLookupComboBox1->KeyValue != -1 && ComboBox1->ItemIndex != -1)
	{
					...
					int id_player = Form1->DBGrid1->DataSource->DataSet->FieldByName("id_player")->AsInteger;
						  bool flag1=true;
...
//---------------------------------------------------------------------------
void __fastcall TForm7::FormCloseQuery(TObject *Sender, bool &CanClose)//close
{
	if(CanClose == true)
	{
	 ...
	 MaskEdit1->Visible=true; ComboBox1->Visible=true; Form7->Edit2->Visible=true; DBLookupComboBox1->Visible=true;
...
//---------------------------------------------------------------------------
void __fastcall TForm7::MaskEdit1KeyDown(TObject *Sender, WORD &Key, TShiftState Shift)
...
//---------------------------------------------------------------------------
void __fastcall TForm7::MaskEdit1KeyPress(TObject *Sender, System::WideChar &Key)

{
	if ( (WideChar(Key)>='A' && WideChar(Key)<='z')) Key=NULL;
}
//---------------------------------------------------------------------------
void __fastcall TForm7::Edit2KeyPress(TObject *Sender, System::WideChar &Key)
{
	if (Key >= '0' && Key <= '9') {
	}
	else if (Key == VK_BACK){}
...
void __fastcall TForm7::BitBtn4Click(TObject *Sender)
{
	...
	if (Edit1->Text.Length() != 0 && Edit3->Text.Length() != 0 &&  DBLookupComboBox2->KeyValue != -1)
	{
			 ...
			 if(flag==false){
				  ...
				  Form4->StatusBar1->Panels->Items[0]->Text="Кількість трансферів - "+IntToStr(kol_transfer);
             }
	}
	else
	{
...
//---------------------------------------------------------------------------
void __fastcall TForm7::DBLookupComboBox2CloseUp(TObject *Sender)
{
	//perevirka
	AnsiString pib = DBLookupComboBox2->ListSource->DataSet->FieldByName(DBLookupComboBox2->ListField)->AsString;
	TDateTime date_1 = Form4->DBGrid1->DataSource->DataSet->FieldByName("date_1")->AsDateTime; bool flag=false;
	for(Form4->DBGrid1->DataSource->DataSet->First(); !Form4->DBGrid1->DataSource->DataSet->Eof; Form4->DBGrid1->DataSource->DataSet->Next())
	{
				if(pib == Form4->DBGrid1->DataSource->DataSet->FieldByName("pib")->AsAnsiString)
				{
...
void __fastcall TForm7::BitBtn6Click(TObject *Sender)
{
	...
	if ( MaskEdit2->Text.Length() != 0 &&  DBLookupComboBox3->KeyValue != -1 &&  DBLookupComboBox4->KeyValue != -1 &&  ComboBox2->ItemIndex != -1 &&  ComboBox3->ItemIndex != -1)
	{
			...
			for(Form3->DBGrid1->DataSource->DataSet->First(); !Form3->DBGrid1->DataSource->DataSet->Eof; Form3->DBGrid1->DataSource->DataSet->Next())
			{
				if(result == Form3->DBGrid1->DataSource->DataSet->FieldByName("result")->AsAnsiString && klub2 == Form3->DBGrid1->DataSource->DataSet->FieldByName("klub2")->AsAnsiString &&
				DateTimePicker4->Date == Form3->DBGrid1->DataSource->DataSet->FieldByName("date_match")->AsDateTime && id_plan+1 == Form3->DBGrid1->DataSource->DataSet->FieldByName("id_plan")->AsInteger &&
				sezon == Form3->DBGrid1->DataSource->DataSet->FieldByName("sezon")->AsAnsiString)
				{
...
//---------------------------------------------------------------------------
void __fastcall TForm7::DBLookupComboBox3CloseUp(TObject *Sender)
{
	//change klub1
...
//---------------------------------------------------------------------------
void __fastcall TForm7::DBLookupComboBox4CloseUp(TObject *Sender)
{
	//change klub2
